...
class GripPosActionClient:
    def __init__(self):
        rclpy.init()
        self.node = rclpy.create_node('grip_pos_action_client')
    ...
    def create_request_for_create_ref_frame(self, target_component):
        ...
        request.frame_name = "Grasping_Position"
        request.parent_frame = target_component
        pose = geometry_msgs.msg.Pose()
        pose.position.x = -0.01755 #delta -0.0351 - pm_demo: -0.01755
        pose.position.y = -0.010935 #delta -0.02186 - pm_demo: -0.01093
        pose.position.z = 0.003 #0.002359 #pm_demo: 0.00235
