...
class MoveToFrameServiceClient(Node):
    def __init__(self, action_block_data):
        
        global unique_node_counter
        node_name = f"move_to_frame_client_node_{unique_node_counter}"
        ...
        #print("Contents of action_block_data:", self.action_block_data) # for debuging purposes
        self.get_logger().info(f"node counter: {unique_node_counter}")

        self.move_group = self.action_block_data["parameters"]["MoveGroup"]

        if self.move_group == "Cam1":
            self.service_name = 'pm_moveit_server/move_cam1_to_frame' #/pm_moveit_server/move_cam1_to_frame
    ...
    def create_request(self):
        ...
        if self.move_group == "Cam1":
            ...
            request.frame_name = self.action_block_data["parameters"]["TargetFrame"]
...
def main(args=None):
    ...
    action_block_data = {
        ...
        'parameters': {
            'MoveGroup': 'Tool',
            'TargetFrame': 'Camera_Station_TCP',
    ...
    move_to_frame_client = MoveToFrameServiceClient(action_block_data)
